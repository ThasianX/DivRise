        ...
        GeometryReader { geometry in
            if self.monthlyRecords.count == 0 {
                ...
                VStack {
                    ...
                    HStack {
                        Spacer()
                        BarChartView(records: self.monthlyRecords, data: self.monthlyDividends, title: "Monthly Dividends", style: Styles.barChartStyleOrangeLight, form: CGSize(width: geometry.size.width*0.95, height: geometry.size.height*0.95))
                        Spacer()
                        //                BarChartView(records: [.mock, .mock, .mock, .mock, .mock, .mock, .mock, .mock, .mock], data: [8,23,54,32,12,37,7,23,43], title: "Monthly Dividends", form: CGSize(width: geometry.size.width*0.95, height: geometry.size.height*0.95))
                    }
