...
public struct LineView: View {
    ...
    public init(records: [Record], data: [Double], title: String? = nil, legend: String? = nil, style: ChartStyle? = Styles.lineChartStyleOne){
        ...
        GeometryReader{ geometry in
            VStack(alignment: .leading, spacing: 8) {
                ...
                ZStack{
                    ...
                    MagnifierRect(currentRecord: self.$currentRecord, currentNumber: self.$currentDataNumber)
...
struct MagnifierRect: View {
    ...
    var body: some View {
        ZStack{
            Text(self.currentRecord.month)
                .font(.system(size: 14, weight: .semibold))
                .offset(x: 0, y:-130)
                .animation(.spring())
                .foregroundColor(self.colorScheme == .dark ? Color.white : Color.black)
            
            Text(self.currentRecord.year)
                .font(.system(size: 14, weight: .semibold))
                .offset(x: 0, y:-115)
                .animation(.spring())
                .foregroundColor(self.colorScheme == .dark ? Color.white : Color.black)
